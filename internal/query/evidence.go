...
func (e *evidenceExecutor) executeEvidence(
	...
	destSpec := nestedMapValue(dataLinkSpec, "dest")
	destKind := stringFromMap(destSpec, "kind")
	destName := stringFromMap(destSpec, "name")
	fieldsMapping := stringNestedMapValue(dataLinkSpec, "fields_mapping")
	...
	storageDestSpec := nestedMapValue(storageLinkSpec, "dest")
	...
	storageProperties := stringNestedMapValue(storageSpec, "properties")
...
func findDataLink(elements []model.UModelElement, entityDomain, entityType, datasetKind string) (model.UModelElement, error) {
	for _, el := range elements {
		...
		src := nestedMapValue(spec, "src")
		dest := nestedMapValue(spec, "dest")
...
func stringNestedMapValue(m map[string]any, key string) map[string]string {
	v, ok := m[key]
	if !ok {
...
// applyPostEvidencePipeline applies post-evidence operators to telemetry rows.
func applyPostEvidencePipeline(rows []map[string]any, columns []string, plan model.QueryPlan) ([]map[string]any, []string) {
	for _, op := range plan.Pipeline {
		...
		switch {
		case op.Name == "project" && len(op.Project) > 0:
			rows, columns = projectRows(rows, op.Project)
		case op.Sort != nil:
			sortRows(rows, *op.Sort)
		case op.Limit > 0:
			rows = limitRows(rows, op.Limit)
		}
	}
	return rows, columns
}
