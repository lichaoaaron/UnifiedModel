...
func TestValidateReturnsErrorForUnknownKind(t *testing.T) {
	...
	_, err := v.Validate(model.MModelElement{Kind: "not_a_real_kind", Domain: "d", Name: "n"})
	if err == nil {
...
func TestNoopValidatorAcceptsEverything(t *testing.T) {
	...
	res, err := v.Validate(model.MModelElement{Kind: "anything_goes", Spec: map[string]any{"garbage": true}})
	if err != nil {
...
func TestValidateEntitySetMinimalSpecAccepted(t *testing.T) {
	...
	for _, spec := range []map[string]any{
		...
		el := model.MModelElement{Kind: "entity_set", Domain: "demo", Name: "demo.x", Spec: spec}
		res, err := v.Validate(el)
		if err != nil {
