...
func (s *Service) Import(ctx context.Context, workspace string, req model.MModelImportRequest) (model.MModelImportResult, error) {
	if workspace == "" {
		return model.MModelImportResult{}, apperrors.New(apperrors.CodeInvalidArgument, "workspace is required")
	}
	if req.Path == "" {
		return model.MModelImportResult{}, apperrors.New(apperrors.CodeInvalidArgument, "import path is required")
	}

	result := model.MModelImportResult{Workspace: workspace, Source: req.Path}
	...
	for _, path := range paths {
		element, err := parseMModelElementFile(path)
		if err != nil {
			return result, apperrors.WithDetails(apperrors.CodeValidationFailed, "mmodel import failed", map[string]string{
				"path":   path,
				"reason": err.Error(),
			})
		}
	...
	write, err := s.PutElements(ctx, model.MModelElementBatch{Workspace: workspace, Elements: elements})
	if err != nil {
		return result, err
	}
...
func (s *Service) loadCommonSchemaPacks(ctx context.Context, workspace string, packs []string) ([]model.MModelElement, error) {
	if len(packs) == 0 {
		return nil, nil
	}
	s.mu.RLock()
...
func parseMModelElementFile(path string) (model.MModelElement, error) {
	body, err := os.ReadFile(path)
	if err != nil {
		return model.MModelElement{}, err
	}
	...
	default:
		return model.MModelElement{}, fmt.Errorf("unsupported file extension")
	}
	return elementFromPayload(normalizeMap(payload))
}

func elementFromPayload(payload map[string]any) (model.MModelElement, error) {
	...
	if kind == "" {
		return model.MModelElement{}, fmt.Errorf("kind is required")
	}
	if domain == "" {
		return model.MModelElement{}, fmt.Errorf("domain or metadata.domain is required")
	}
	if name == "" {
		return model.MModelElement{}, fmt.Errorf("name or metadata.name is required")
	}
