...
func TestValidateRejectsInvalidMModelElement(t *testing.T) {
	svc := NewService(graphstore.NewMemoryStore())
	result, err := svc.Validate(context.Background(), "demo", []model.MModelElement{{Kind: "entity_set"}})
	if err != nil {
...
func TestValidateAcceptsValidMModelElements(t *testing.T) {
	svc := NewService(graphstore.NewMemoryStore())
	result, err := svc.Validate(context.Background(), "demo", []model.MModelElement{{
		Kind:   "entity_set",
		Domain: "devops",
		Name:   "devops.service",
...
`)
	...
	result, err := svc.Import(ctx, "demo", model.MModelImportRequest{Path: path})
	if err != nil {
...
`)
	...
	first, err := svc.Import(ctx, "demo", model.MModelImportRequest{Path: path})
	if err != nil {
		t.Fatalf("first import: %v", err)
	}
	second, err := svc.Import(ctx, "demo", model.MModelImportRequest{Path: path})
	if err != nil {
...
`)
  ...
  "spec": {"fields": [{"name": "service_id"}]}
	...
	svc := NewService(graphstore.NewMemoryStore())
	result, err := svc.Import(ctx, "demo", model.MModelImportRequest{Path: dir})
	if err != nil {
...
func TestImportRejectsInvalidSourcesWithStableErrors(t *testing.T) {
	...
	if _, err := svc.Import(ctx, "demo", model.MModelImportRequest{}); !apperrors.IsCode(err, apperrors.CodeInvalidArgument) {
		t.Fatalf("expected missing path invalid argument, got %v", err)
	}
	if _, err := svc.Import(ctx, "demo", model.MModelImportRequest{Path: filepath.Join(t.TempDir(), "missing.yaml")}); !apperrors.IsCode(err, apperrors.CodeInvalidArgument) {
		t.Fatalf("expected missing file invalid argument, got %v", err)
	}
	txt := writeTestFile(t, "README.txt", "not a schema")
	if _, err := svc.Import(ctx, "demo", model.MModelImportRequest{Path: txt}); !apperrors.IsCode(err, apperrors.CodeInvalidArgument) {
		t.Fatalf("expected unsupported extension invalid argument, got %v", err)
	}
	badYAML := writeTestFile(t, "bad.yaml", "kind: [")
	if _, err := svc.Import(ctx, "demo", model.MModelImportRequest{Path: badYAML}); !apperrors.IsCode(err, apperrors.CodeValidationFailed) {
...
func TestImportCommonSchemaPackHookIsExplicit(t *testing.T) {
	svc := NewService(graphstore.NewMemoryStore())
	_, err := svc.Import(context.Background(), "demo", model.MModelImportRequest{
