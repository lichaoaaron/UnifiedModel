...
import (
	...
	"github.com/alibaba/MModel/internal/mmodel/schemaspec"
...
// validation on MModel elements. schemaspec.Validator implements it.
type SchemaValidator interface {
	Validate(element model.MModelElement) (schemaspec.Result, error)
}
...
type CommonSchemaLoader interface {
	LoadCommonSchemaPacks(ctx context.Context, workspace string, packs []string) ([]model.MModelElement, error)
}
...
func (s *Service) Validate(ctx context.Context, workspace string, elements []model.MModelElement) (model.ValidationResult, error) {
...
func (s *Service) PutElements(ctx context.Context, batch model.MModelElementBatch) (model.WriteResult, error) {
	if batch.Workspace == "" {
	...
	if !validation.Valid {
		...
		return model.WriteResult{}, apperrors.WithDetails(apperrors.CodeValidationFailed, "mmodel validation failed", details)
	}
	result, err := s.graph.PutMModelElements(ctx, batch)
	if err != nil {
		return model.WriteResult{}, err
	}
...
func (s *Service) RebuildIndex(ctx context.Context, workspace string) error {
	...
	snapshot, err := s.graph.GetMModelSnapshot(ctx, model.MModelSnapshotRequest{Workspace: workspace})
	if err != nil {
		return err
	}
