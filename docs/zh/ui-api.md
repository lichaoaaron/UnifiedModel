...
English: [MModel Web UI API Map](../en/ui-api.md)
...
| Explorer | 加载模型图/表 | `POST /api/v1/query/{workspace}/execute` | `{"query": ".mmodel | sort id | limit 100", "limit": 100}` |
| Explorer 详情 | 校验元素 JSON | `POST /api/v1/mmodel/{workspace}/validate` | `{"elements": [MModelElement]}` |
| Explorer 详情 | 保存元素 JSON | `POST /api/v1/mmodel/{workspace}/elements` | `{"elements": [MModelElement]}` |
| Explorer 详情 | 删除元素 | `DELETE /api/v1/mmodel/{workspace}/elements` | `{"ids": ["element-id"]}` |
...
| Imports | 导入服务端路径 | `POST /api/v1/mmodel/{workspace}/import` | `MModelImportRequest` |
| Imports | 导入内置 quickstart 样例 | `POST /api/v1/samples/{workspace}/multi-domain-quickstart:import` | 无 |
| Imports | 校验 MModel JSON | `POST /api/v1/mmodel/{workspace}/validate` | `{"elements": [...]}` |
| Imports | 写入 MModel JSON | `POST /api/v1/mmodel/{workspace}/elements` | `{"elements": [...]}` |
...
| Agent | 读取 Resource | `POST /api/v1/agent/{workspace}/resources:read` | `{"uri": "mmodel://..."}` |
...
- 开源 UI 中 MModel forms 为 JSON-only。未来的 schema-aware editors 继续复用同一组 `validate` 和 `elements` APIs。
