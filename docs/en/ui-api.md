...
| Explorer | Load model graph/table | `POST /api/v1/query/{workspace}/execute` | `{"query": ".mmodel | sort id | limit 100", "limit": 100}` |
| Explorer detail | Validate element JSON | `POST /api/v1/mmodel/{workspace}/validate` | `{"elements": [MModelElement]}` |
| Explorer detail | Save element JSON | `POST /api/v1/mmodel/{workspace}/elements` | `{"elements": [MModelElement]}` |
| Explorer detail | Delete element | `DELETE /api/v1/mmodel/{workspace}/elements` | `{"ids": ["element-id"]}` |
...
| Imports | Import server path | `POST /api/v1/mmodel/{workspace}/import` | `MModelImportRequest` |
| Imports | Import bundled quickstart sample | `POST /api/v1/samples/{workspace}/multi-domain-quickstart:import` | none |
| Imports | Validate MModel JSON | `POST /api/v1/mmodel/{workspace}/validate` | `{"elements": [...]}` |
| Imports | Put MModel JSON | `POST /api/v1/mmodel/{workspace}/elements` | `{"elements": [...]}` |
...
| Agent | Read resource | `POST /api/v1/agent/{workspace}/resources:read` | `{"uri": "mmodel://..."}` |
...
- MModel forms are JSON-only in the open-source UI. Future schema-aware editors can be added behind the same `validate` and `elements` APIs.
