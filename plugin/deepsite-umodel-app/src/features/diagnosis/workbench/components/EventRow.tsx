...
import { phaseLabel } from '../utils/diagnosisEventMapper'
        ...
        <div className="ml-[18px] pl-[36px] py-2 pr-4 border-l border-[#e5e7eb] text-[12px] leading-[1.55] text-[#4b5563] animate-fadeIn">
          ...
          {showReport && (
            <DetailSection label="报告内容">
              <DiagnosisReport content={(event.raw as { content: string }).content} entityData={entityData} />
            </DetailSection>
          )}

          {showInlineTopo && (
            <DetailSection label="拓扑">
              <ServiceCallGraph graph={event.raw as CallGraph} />
            </DetailSection>
          )}
        </div>
      )}
