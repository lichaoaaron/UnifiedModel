...
const examples = [
  {
    labelKey: 'query.examples.umodel',
    query: ".umodel with(kind='entity_set') | project domain,name,kind | sort domain,name | limit 1000",
  },
  {
...
export function QueryPage({ api, workspaceId }: { api: UModelApi; workspaceId: string }) {
  ...
  const resultColumns = tableResult?.columns.length
    ? tableResult.columns
    : tableResult?.rows[0]
      ? Object.keys(tableResult.rows[0])
      : [];
  const topoData = useMemo(
    () => (result ? buildEntityTopoData(result, [], topoEntityRows) : null),
    [result, topoEntityRows]
  );
    ...
    try {
      ...
      if (kind === 'execute') {
        const [next, nextExplain] = await Promise.all([
          api.query(workspaceId, request),
          api.explain(workspaceId, request),
        ]);
      ...
      {explainOpen && (
        <div className="query-explain-drawer" role="presentation" onClick={() => setExplainOpen(false)}>
          ...
          >
            ...
            <MonacoBlock value={explain ? stringify(explain) : '{}'} language="json" height={260} readOnly />
          </section>
        </div>
      )}
    </div>
  ...
  return (
    <div className="query-monaco" style={{ height }}>
      <CodeEditor
        ...
        height="100%"
        readOnly={readOnly}
        showLineNumbers={lineNumbers === 'on'}
        showMiniMap={false}
      ...
      <div className="query-chart-controls">
        ...
        <div className="query-chart-filter-wrap">
          <button
            className={
              activeFilterCount > 0
                ? 'eto-filter-chip query-chart-filter-trigger active'
                : 'eto-filter-chip query-chart-filter-trigger'
            }
          ...
          >
            ...
            <tbody>
              {pageRows.map((row, index) => {
                const rowPresentations = columns.map((column, columnIndex) =>
                  getCellPresentation(row[column], tableLayout.columnWidths[columnIndex])
                );
                ...
                return (
                  <tr key={`${safePage}-${index}`} className={rowHasPreviewable ? 'query-row-has-code' : undefined}>
                    {columns.map((column, columnIndex) => {
                      ...
                      const showFullHover =
                        presentation.showFullHover &&
                        (!wrapLongText ||
                          !canWrappedTextShowFull(presentation.full, tableLayout.columnWidths[columnIndex]));
                      ...
                      return (
                        <td key={column} className={cellClassName || undefined}>
                          <div
                            className={
                              presentation.previewable ? 'query-cell-content previewable' : 'query-cell-content'
                            }
                          >
        ...
        <footer className="query-table-footer">
          <div className="query-table-pages">
            ...
            {pageNumbers.map((item, index) =>
              item === '...' ? (
                <span className="query-table-page-gap" key={`gap-${index}`}>
                  ...
                </span>
              ) : (
                <button
      ...
      {preview && (
        <aside className="query-preview-panel">
          ...
          <div className="query-preview-body">
            <MonacoBlock value={preview.value} language={preview.language} height="100%" readOnly wordWrap="on" />
          </div>
        </aside>
      )}
    </div>
...
async function loadTopoEntityRows(
  ...
  const results = await Promise.all(
    [...idsByType.values()].map(async (group) => {
