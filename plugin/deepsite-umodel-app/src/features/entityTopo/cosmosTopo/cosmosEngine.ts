...
export class CosmosEngine {
  ...
  getClusterLabelPositionsMap(): ReadonlyMap<string, [number, number]> {
    return this.clusterLabelPositions;
  }
  ...
  private findLinkAtScreenPositionFromGraph(
    event: MouseEvent,
    maxDistance = LINK_CLICK_HIT_DISTANCE
  ): number | undefined {
    if (!this.graph) {return undefined;}
    const [x, y] = this.getEventOffset(event);
    let positions: number[];
