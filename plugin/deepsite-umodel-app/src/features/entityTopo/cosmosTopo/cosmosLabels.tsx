...
export function CosmosLabels({
  ...
  const syncLabels = useCallback(() => {
    ...
    const shouldRenderZoomNodeLabels =
      showLabels && !shouldRenderAllNodeLabels && !shouldRenderFocusedNodeLabels && zoom >= ZOOM_TIER_MID;
    ...
    if (!showLabels && !showClusterLabels) {
      ...
      if (showClusterLabels)
        {renderClusterLabelItems(clusterItems, clusters, engine, graph, positions, isVisible, occupied, clusterLimit);}
      if (showLabels) {
        renderNodeLabels(
          elements,
          nodes,
...
function renderClusterLabelItems(
  ...
  for (const c of sorted) {
    ...
    if (
      !reserveRect(occupied, { x: x - labelWidth / 2, y: y - labelHeight / 2, width: labelWidth, height: labelHeight })
    )
      {continue;}
