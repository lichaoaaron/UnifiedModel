...
function CosmosVectorIconOverlay({ engine, tick }: { engine: CosmosEngine | null; tick: number }) {
  // `tick` forces a re-read of the imperative engine's overlay state each frame;
        ...
        >
          {(info.rows ?? []).map(([key, value]) => (
            <React.Fragment key={key}>
              <span style={{ color: 'var(--om-text-muted)' }}>{key}</span>
              <span
                style={{
                  color: 'var(--om-text)',
                  fontWeight: 650,
...
export const CosmosTopoGraph = forwardRef<TopoGraphRef, Omit<TopoGraphProps, 'type'>>((props, ref) => {
  ...
  useEffect(() => {
    ...
    // The engine is (re)built only when these layout knobs change; the stable
    // finishRendering/requestTick callbacks are intentionally excluded so the
    // engine is not torn down and recreated on every render.
  }, [
    layout.mode,
