...
const docDescriptionKeys: Record<string, string> = {
  ...
  'Optional from/to timestamps for temporal entity and topology reads.':
    'apiDebugger.doc.optionalFromToTimestampsForTemporalEntityAndTopologyReads',
  ...
  'Per-item result details, including validation errors when present.':
    'apiDebugger.doc.perItemResultDetailsIncludingValidationErrorsWhenPresent',
  ...
  'Set true to include conflicted workspace identities.':
    'apiDebugger.doc.setTrueToIncludeConflictedWorkspaceIdentities',
  ...
  'When true, replace all custom config instead of merging.':
    'apiDebugger.doc.whenTrueReplaceAllCustomConfigInsteadOfMerging',
  ...
  'Workspace ID. The debugger fills it from the current workspace by default.':
    'apiDebugger.doc.workspaceIdFilledFromCurrentWorkspace',
...
const apiSpecs: ApiSpec[] = [
    ...
    params: [
      {
        ...
        in: 'query',
        type: 'number',
        description: 'Maximum items returned by this page.',
        defaultValue: '100',
      },
      {
        name: 'page_token',
      ...
      },
      {
        ...
        in: 'query',
        type: 'boolean',
        description: 'Set true to include tombstoned workspaces.',
      },
      {
        name: 'include_conflicts',
        in: 'query',
        type: 'boolean',
        description: 'Set true to include conflicted workspace identities.',
      },
    ],
    responseFields: [
  ...
  },
  {
    ...
    bodyFields: [
      ...
      { name: 'replace_labels', type: 'boolean', description: 'When true, replace all labels instead of merging.' },
      {
        name: 'replace_config',
        type: 'boolean',
        description: 'When true, replace all custom config instead of merging.',
      },
    ],
  ...
  },
  {
    ...
    bodyFields: [
      ...
      { name: 'timeout_ms', type: 'number', description: 'Optional execution timeout.' },
      {
        name: 'time_range',
        type: 'object',
        description: 'Optional from/to timestamps for temporal entity and topology reads.',
      },
  ...
  },
  {
    ...
    risk: 'read',
    params: [workspaceParam],
    bodyFields: [{ name: 'elements', type: 'UModelElement[]', required: true, description: 'Elements to validate.' }],
  ...
  },
  {
    ...
    risk: 'write',
    params: [workspaceParam],
    bodyFields: [{ name: 'elements', type: 'UModelElement[]', required: true, description: 'Elements to write.' }],
    defaultBody: () => ({ elements: [sampleUModelElement()] }),
  ...
  },
  {
    ...
    risk: 'danger',
    params: [workspaceParam],
    bodyFields: [{ name: 'ids', type: 'string[]', required: true, description: 'Element IDs to delete.' }],
  ...
  },
  {
    ...
    params: [
      workspaceParam,
      {
        ...
        required: true,
        type: 'string',
        description: 'Bundled sample name.',
        defaultValue: 'multi-domain-quickstart',
      },
    ],
    responseFields: [
      { name: 'workspace', type: 'string', required: true, description: 'Workspace ID.' },
  ...
  },
  {
    ...
    defaultBody: () => ({
      ids: [
        `${sampleDomain}/${sampleEntityType}/${sampleServiceAId}/${sampleRelationType}/${sampleDomain}/${sampleEntityType}/${sampleServiceBId}`,
      ],
      reason: 'manual debug',
    }),
    responseFields: writeResultFields,
  ...
  },
  {
    ...
    risk: 'read',
    params: [workspaceParam],
    bodyFields: [{ name: 'uri', type: 'string', required: true, description: 'Resource URI from discovery.' }],
...
export function ApiMapPage({ api, workspaceId }: { api: UModelApi; workspaceId: string }) {
  ...
  useEffect(() => {
    ...
    setResult(null);
    setError('');
    // Reset the request draft only when the selected spec (by id) or the
    // workspace changes — not on every new `selected` object identity.
  ...
  async function run() {
    ...
    try {
      ...
      // Route through the Go proxy (getBackendSrv) like every other backend call,
      // so the debugger inherits the Grafana session and server-side key injection.
      // rawRequest returns non-2xx outcomes instead of throwing, so we surface the
      // real status/duration rather than a toast.
      const response = await api.rawRequest(selected.method, requestUrl, payload);
      setResult({
      ...
      <div className="api-debug-layout">
        ...
        <section className="api-debug-response">
          ...
          <div className="api-debug-response-json">
            <MonacoBlock value={result ? stringify(result.payload) : '{}'} language="json" height="100%" readOnly />
          </div>
...
function MethodBadge({ method, compact = false }: { method: HttpMethod; compact?: boolean }) {
  return (
    <span className={`api-debug-method method-${method.toLowerCase()} ${compact ? 'compact' : ''}`}>{method}</span>
  );
}
