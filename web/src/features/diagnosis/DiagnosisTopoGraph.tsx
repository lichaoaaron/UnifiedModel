...
import type { CallGraph, CallNode, CallEdge } from './diagnosisTypes'
...
export function DiagnosisTopoGraph({ graph, onNodeClick, selectedNodeId }: Props) {
  const [collapsed, setCollapsed] = useState(false)

  if (!graph || graph.nodes.length === 0) return null
  if (collapsed) {
    return (
      <div style={{ background: 'var(--color-surface)', borderRadius: 12, border: '1px solid var(--color-border)', padding: 16, boxShadow: '0 1px 5px rgba(15,23,42,0.06)' }}>
        <button
          type="button"
          onClick={() => setCollapsed(false)}
          aria-expanded="false"
          title="展开本体层拓扑"
          style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, border: 0, background: 'transparent', padding: 0, textAlign: 'left', cursor: 'pointer' }}
        >
          <span style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 12, fontWeight: 600, color: 'var(--color-primary)' }}>
  ...
  return (
    <div style={{ background: 'var(--color-surface)', borderRadius: 12, border: '1px solid var(--color-border)', padding: 16, boxShadow: '0 1px 5px rgba(15,23,42,0.06)' }}>
      <button
        ...
        aria-expanded="true"
        title="收起本体层拓扑"
        style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, border: 0, background: 'transparent', padding: 0, marginBottom: 12, textAlign: 'left', cursor: 'pointer' }}
      >
        <span style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 12, fontWeight: 600, color: 'var(--color-primary)' }}>
          <ChevronDown size={14} />
          <span>本体层拓扑</span>
      ...
      {/* Legend */}
      <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 12, marginBottom: 12, fontSize: 10 }}>
        <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
          <span style={{ display: 'inline-block', width: 12, height: 12, borderRadius: 2, border: '1px solid #3b82f6', background: '#dbeafe' }} />
          <span style={{ color: '#1d4ed8' }}>入口</span>
        </span>
        <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
          <span style={{ display: 'inline-block', width: 12, height: 12, borderRadius: 2, border: '2px solid #ef4444', background: '#fee2e2' }} />
          <span style={{ color: '#991b1b' }}>根因</span>
        </span>
        <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
          <span style={{ display: 'inline-block', width: 12, height: 12, borderRadius: 2, border: '1px solid #eab308', background: '#fef9c3' }} />
          <span style={{ color: '#854d0e' }}>受影响</span>
        </span>
        <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
          <span style={{ display: 'inline-block', width: 12, height: 12, borderRadius: 2, border: '1px solid #d1d5db', background: '#f9fafb' }} />
          <span style={{ color: '#4b5563' }}>本体节点</span>
        </span>
        {hasStormRoles && (
          <>
            <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
              <span style={{ display: 'inline-block', width: 12, height: 12, borderRadius: 2, border: '1px solid #d97706', background: '#fef3c7' }} />
              <span style={{ color: '#92400e' }}>次要</span>
            </span>
            <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
              <span style={{ display: 'inline-block', width: 12, height: 12, borderRadius: 2, border: '1px solid #9ca3af', background: '#f8fafc', opacity: 0.75 }} />
              <span style={{ color: '#6b7280' }}>背景</span>
            </span>
          </>
        )}
      </div>
