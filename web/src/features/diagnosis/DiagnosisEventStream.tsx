...
import DiagnosisEventRow from './DiagnosisEventRow'
...
export function DiagnosisEventStream({ events, loading, highlightedNodeId, onNodeClick, focusedEventId }: Props) {
  ...
  const hasActiveGeneration = events.some(
    e => e.status === 'running' || (e.kind === 'text' && e.status === 'info' && events.indexOf(e) === events.length - 1)
  )
  ...
  return (
    ...
    >
      <div className="event-stream-header flex items-center gap-2 px-3 py-1.5 border-b border-[#f0f0f0] bg-[#fafafa] sticky top-0 z-10">
