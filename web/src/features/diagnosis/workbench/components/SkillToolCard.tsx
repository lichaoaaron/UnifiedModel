...
const MMODEL_PHRASES: Record<string, string> = {
...
function mmodelPhrase(msg: DiagnosisMessage): string {
  const skillKey = msg.skill_name ?? msg.tool_name?.split('/').pop() ?? ''
  return MMODEL_PHRASES[skillKey] ?? msg.content ?? skillKey
}
  ...
  return (
    <div className="rounded-md border border-[#dbe7f3] bg-white text-sm overflow-hidden shadow-[0_1px_5px_rgba(15,23,42,0.06)]">
      ...
      >
        ...
        <span className="text-[#0e639c] font-mono text-sm font-semibold">MModel：{phrase}</span>
        <span className="text-[#9ca3af] font-mono text-sm">·</span>
