...
func TestImportQuickstartExamplesAndQueryMModel(t *testing.T) {
	ctx := context.Background()
	app := bootstrap.NewMemoryApp(t.TempDir())
	examples := filepath.Join("..", "..", "examples", "quickstart-multidomain")

	result, err := app.MModel.Import(ctx, "demo", model.MModelImportRequest{Path: examples})
	if err != nil {
	...
	bySpec, err := app.Query.Execute(ctx, "demo", model.QueryRequest{
		Query: ".mmodel with(kind='entity_set', domain='devops', query='Service') | limit 10",
	})
	if err != nil {
