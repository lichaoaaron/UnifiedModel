...
使用方法：
    # 验证单个文件
    python mmodel_validator.py <config_file>
    python mmodel_validator.py examples/dataset/metricset/sls.front.metric.yaml
    
    # 批量验证目录（递归）
    python mmodel_validator.py --batch <directory>
    python mmodel_validator.py --batch examples/
    
    # 批量验证目录（非递归）
    python mmodel_validator.py --batch <directory> --no-recursive
    
    # 启用控制台日志输出
    python mmodel_validator.py --console-log <config_file>
"""
...
class ConfigValidator(SchemaValidator):
    ...
    def __init__(self, expanded_schemas_dir: str = "expanded_schemas", base_schema_path: str = "schemas/base.yaml", console_log: bool = False):
        ...
        self.logger = logging.getLogger('mmodel_validator')
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="MModel 配置文件验证器",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
使用示例:
  # 验证单个文件
  python mmodel_validator.py config.yaml
  
  # 批量验证目录（递归）
  python mmodel_validator.py --batch examples/
  
  # 批量验证目录（非递归）
  python mmodel_validator.py --batch examples/ --no-recursive
  
  # 启用控制台日志输出
  python mmodel_validator.py --console-log config.yaml
        """
    )
