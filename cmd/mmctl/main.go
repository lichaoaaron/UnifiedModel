...
func run(args []string, out, errOut io.Writer) error {
	flags := flag.NewFlagSet("mmctl", flag.ContinueOnError)
	flags.SetOutput(errOut)
	addr := flags.String("addr", "http://localhost:8080", "MModel server address")
	...
	case "serve":
		fmt.Fprintln(out, "Use `mmodel-server --quickstart` for the in-memory demo, `--graphstore file.memory` for local persistence, or build with `-tags ladybug` for local.ladybug storage.")
		return nil
	default:
...
func (c cli) mmodel(args []string) error {
	if len(args) == 0 {
		return fmt.Errorf("mmodel command is required")
	}
	if forbidden(args[0], "get", "list", "graph") {
		return fmt.Errorf("mmodel %s is forbidden; read MModel data through query run/explain", args[0])
	}
	switch args[0] {
	case "import":
		if len(args) < 3 {
			return fmt.Errorf("usage: mmodel import <workspace> <yaml-json-file-or-directory>")
		}
		return c.doJSON(http.MethodPost, "/api/v1/mmodel/"+args[1]+"/import", map[string]any{"path": args[2]})
	case "put", "validate":
		if len(args) < 3 {
			return fmt.Errorf("usage: mmodel %s <workspace> <json-file-or-inline>", args[0])
		}
		payload, err := readJSONArg(args[2])
		if err != nil {
			return err
		...
		return c.doRaw(http.MethodPost, "/api/v1/mmodel/"+args[1]+"/"+action, payload)
	case "delete":
		if len(args) < 3 {
			return fmt.Errorf("usage: mmodel delete <workspace> <ids-json-csv-or-payload>")
		}
		payload, err := idsPayload(args[2:], "")
		if err != nil {
			return err
		}
		return c.doRaw(http.MethodDelete, "/api/v1/mmodel/"+args[1]+"/elements", payload)
	case "export":
		if len(args) < 2 {
			return fmt.Errorf("usage: mmodel export <workspace> [limit]")
		}
		limit := 1000
		if len(args) >= 3 {
		...
		return c.doJSON(http.MethodPost, "/api/v1/query/"+args[1]+"/execute", map[string]any{
			"query": fmt.Sprintf(".mmodel | limit %d", limit),
			"limit": limit,
		})
	default:
		return fmt.Errorf("unknown mmodel command: %s", args[0])
	}
}
...
func (c cli) query(args []string) error {
	...
	case "examples":
		fmt.Fprintln(c.out, `.mmodel with(kind='entity_set') | limit 20`)
...
func (c cli) agent(args []string) error {
	...
	switch args[0] {
	case "mcp":
		fmt.Fprintln(c.out, "Use `mmodel-mcp` for the stdio MCP server.")
		return nil
	case "discover":
		if len(args) < 2 {
...
func reasonFromArgs(action string, args []string) string {
	...
	if action == "delete" {
		return "delete requested by mmctl"
	}
	return "expire requested by mmctl"
}
...
func printHelp(out io.Writer) {
	fmt.Fprintln(out, `mmctl

Command groups:
  workspace create|get|list|update|delete
  mmodel put|delete|import|export|validate
