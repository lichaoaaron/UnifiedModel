...
func run(args []string, in io.Reader, out, errOut io.Writer) error {
	flags := flag.NewFlagSet("mmodel-mcp", flag.ContinueOnError)
	flags.SetOutput(errOut)
	workspace := flags.String("workspace", "demo", "default workspace for MCP requests")
	dataRoot := flags.String("data", ".mmodel-data", "local MModel data root")
	...
	*provider = resolveProviderForQuickStart(*provider, *quickStart, graphstoreExplicit)
	...
	if *quickStart {
		...
		fmt.Fprintf(errOut, "quickstart loaded workspace=%s sample=%s mmodel_imported=%d mmodel_skipped=%d entities=%d relations=%d\n",
			result.Workspace,
			result.Sample,
