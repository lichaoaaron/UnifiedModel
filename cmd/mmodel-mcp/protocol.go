...
func handleRequest(ctx context.Context, app *bootstrap.App, defaultWorkspace string, req rpcRequest) (any, *rpcError) {
	...
	case "discovery", "mmodel/discovery":
		discovery, err := app.AgentGateway.Discover(ctx, workspace)
		if err != nil {
			return nil, internalRPCError(err)
		}
...
func initializeResult(ctx context.Context, app *bootstrap.App, workspace string, req rpcRequest) (any, *rpcError) {
	...
	return map[string]any{
		...
		"serverInfo": map[string]string{
			"name":    "mmodel-mcp",
			"title":   "MModel MCP Server",
			"version": "0.1.0",
		},
		"instructions": "Use MModel query tools for .mmodel, .entity, and .topo reads. Tool and resource text payloads are encoded as TOON while the MCP JSON-RPC envelope remains JSON.",
		"discovery":    discovery,
		"_meta": map[string]any{
			"workspace":    workspace,
...
func mcpResourceTemplates(workspace string) []map[string]any {
	...
	for _, name := range templates {
		out = append(out, map[string]any{
			...
			"uriTemplate": "mmodel://workspace/{workspace}/" + name,
			"description": fmt.Sprintf("Read %s metadata for a MModel workspace.", name),
			"mimeType":    toonMimeType,
			"_meta": map[string]any{
...
func mcpPrompts() []map[string]any {
	return []map[string]any{
		{
			"name":        "mmodel_query_context",
			"title":       "MModel Query Context",
			"description": "Prepare a MModel query task using .mmodel, .entity, and .topo surfaces.",
		...
		{
			"name":        "mmodel_object_graph_review",
			"title":       "MModel Object Graph Review",
...
func getPrompt(defaultWorkspace string, params map[string]any) (any, *rpcError) {
	...
	switch name {
	case "mmodel_query_context":
		query := stringArg(args, "query")
		if query == "" {
			query = ".mmodel | limit 20"
		}
		return promptResult("Use MModel Query Service", fmt.Sprintf("Workspace: %s\nRun or refine this MModel SPL through query_spl_execute or query_spl_explain:\n%s\n\nPrefer .mmodel, .entity, and .topo as the public read sources. Tool/resource data returned by this server is encoded as TOON.", workspace, query)), nil
	case "mmodel_object_graph_review":
		focus := stringArg(args, "focus")
		if focus == "" {
			focus = "model definitions, runtime entities, and topology relations"
		}
		return promptResult("Review MModel Object Graph Context", fmt.Sprintf("Workspace: %s\nFocus: %s\nUse resources for metadata, then query tools for runtime rows. Keep resources metadata-only and use Query Service for .mmodel, .entity, and .topo reads.", workspace, focus)), nil
	default:
...
func completionResult(workspace string, params map[string]any) map[string]any {
	...
	case "ref/resource":
		for _, name := range []string{"overview", "schema-index", "query-templates", "tool-capability-metadata"} {
			candidate := "mmodel://workspace/" + workspace + "/" + name
			if value == "" || strings.Contains(strings.ToLower(candidate), value) || strings.Contains(name, value) {
				values = append(values, candidate)
			}
		}
	case "ref/prompt":
		for _, candidate := range []string{"mmodel_query_context", "mmodel_object_graph_review", workspace, ".mmodel | limit 20", ".entity | limit 20", ".topo | limit 20"} {
