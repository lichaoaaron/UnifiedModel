...
class ImpactAnalysisSkill(BaseSkill):
    ...
    def run(self, ctx: DiagnosisContext) -> SkillResult:
        ...
        runtime_call_edges = [edge for edge in edges if edge.get("label") == "calls"]
        call_services = []
        for edge in runtime_call_edges:
            for svc in (edge.get("source", ""), edge.get("target", "")):
        ...
        return SkillResult(
            ...
            summary=(
                ...
                f"业务语义影响（演示本体）：{biz_summary}；"
                f"影响规模：暂不估算（未接入真实业务指标）。"
            ),
            started_at=started_at,
            finished_at=finished_at,
            duration_ms=duration_ms,
            input={"root_cause_service": root_cause_service, "call_graph_edges": len(edges)},
