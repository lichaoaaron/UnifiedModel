"""
GraphAnalysisSkill: builds the current diagnosis runtime subgraph from trace/log/metric evidence.
"""
import logging
...
class GraphAnalysisSkill(BaseSkill):
    ...
    def run(self, ctx: DiagnosisContext) -> SkillResult:
        ...
        spans = [span.get("_source", span) for span in adapter.get_traces(query_context=ctx.query_context, data_dir=ctx.data_dir, case_id=ctx.case_id)]
        ...
        impact_path = []
        if first_error_svc:
            impact_path = [first_error_svc]
            for svc in services:
                if svc != first_error_svc:
                    impact_path.append(svc)
            if ctx.api:
        ...
        return SkillResult(
            ...
            status="success",
            summary=(
                f"已加载 MModel 本体（{mmodel_entity_type_count} 个实体类型），"
                f"基于本次观测数据构建运行态子图。"
            ),
