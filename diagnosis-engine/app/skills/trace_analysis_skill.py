...
from app.runtime.trace_input_resolver import resolve_trace_input as _resolve_trace_input
...
class TraceAnalysisSkill(BaseSkill):
    ...
    def _choose_trace_id(self, spans: list[dict], entry_api: str, fault_time: str, entity_services: set[str] | None = None) -> str | None:
        ...
        def _rank(item: tuple[str, dict[str, object]]) -> tuple[int, float, str]:
            ...
            if profile["has_error"]:
                score += 12  # heavily prefer traces with errors
            ...
            if profile["has_rpc"]:
                score += 1
            # Boost traces that include entity-bound services.
            # This aligns trace selection with services known to be relevant.
            if entity_services:
                matching = len(profile["services"] & entity_services)
                score += matching * 2
            return (
                score,
    ...
    def _hop_to_downstream_trace(self, ctx: DiagnosisContext, spans: list[dict], trace_id: str) -> tuple[list[dict], str | None]:
        ...
        for span in spans:
            ...
            status_code = src.get("status.code") or (src.get("status") or {}).get("code")
            if str(status_code) == "2":
                trace_has_error_spans = True
            if trace_has_unavailable_signal and trace_has_error_spans:
                break

        # Trigger downstream hop if current trace has no errors at all —
        # the root cause evidence may be in a different trace.
        if not trace_has_unavailable_signal and not trace_has_error_spans:
            trace_has_unavailable_signal = True  # force hop to find error traces
    ...
    def run(self, ctx: DiagnosisContext) -> SkillResult:
        ...
        # ── DCC-first: check whether DCC already supplies trace spans ──────────
        resolution = _resolve_trace_input(ctx)
        for w in resolution.warnings:
            execution_log.append(w)

        # ── Entity-guided trace selection: if EntityBinding already ran, prefer
        # traces from entity-bound services.
        entity_services: set[str] = set(ctx.entity_result.get("services", []) if isinstance(ctx.entity_result, dict) else [])

        if resolution.skipped_adapter:
            # Priority 1: use spans from DCC evidence.trace.items
            data_dir = "dcc"
            spans = list(resolution.spans)
            execution_log.append(
                f"[DCC-first] 使用 DCC evidence.trace.items，共 {len(spans)} 个 span"
            )
            # Apply best-trace selection within the DCC span set (same as OpenSearch mode)
            selected_trace_id = self._choose_trace_id(spans, ctx.api, ctx.time, entity_services=entity_services)
            if selected_trace_id:
                spans = [
                    s for s in spans
                    if (s.get("_source") or s).get("traceId") == selected_trace_id
                ]
                ctx.query_context["trace_id"] = selected_trace_id
                execution_log.append(f"收敛到代表性 traceId: {selected_trace_id}")
            # _hop_to_downstream_trace is skipped in DCC mode (it calls adapter.get_traces)
        else:
            # Priority 2 / 3: legacy adapter / case loader path
            data_source = adapter.get_data_source()
            if data_source == "opensearch":
                data_dir = "opensearch"
            elif data_source == "local_json":
                data_dir = resolve_data_dir(data_dir=ctx.data_dir, case_id=ctx.case_id)
            else:
                data_dir = "unifiedmodel"

            execution_log.append("查询 OpenSearch trace" if data_dir == "opensearch" else ("读取 UnifiedModel traces" if data_dir == "unifiedmodel" else f"读取 {data_dir}/trace.json"))
            ...
            selected_trace_id = self._choose_trace_id(spans, ctx.api, ctx.time, entity_services=entity_services) if data_dir == "opensearch" else None
            if selected_trace_id:
                refined_query_context = dict(ctx.query_context)
                ...
                # ── Broad fallback: if current trace has zero or very few error
                # spans, scan ALL traces in the time window for better candidates.
                error_count = sum(
                    1 for s in spans
                    if str(s.get("_source", s).get("status.code", "")) == "2"
                    or str(s.get("_source", s).get("span.attributes.http@status_code", "")).startswith("5")
                )
                if error_count < 10 and not hopped_trace_id:
                    fallback_qc = dict(ctx.query_context)
                    fallback_qc.pop("trace_id", None)
                    fallback_qc["alert_api"] = None
                    fallback_qc["api"] = None
                    fallback_qc["limit"] = max(int(fallback_qc.get("limit") or 0), 2000)
                    all_spans = adapter.get_traces(query_context=fallback_qc, data_dir=ctx.data_dir, case_id=ctx.case_id)
                    error_spans_by_trace: dict[str, list[dict]] = {}
                    for s in all_spans:
                        src = s.get("_source", s)
                        tid = src.get("traceId")
                        if not tid:
                            continue
                        if str(src.get("status.code", "")) == "2":
                            error_spans_by_trace.setdefault(tid, []).append(s)
                    if error_spans_by_trace:
                        # Prefer traces that also contain the alert API (in ANY span,
                        # not just error spans), then entity-bound error services,
                        # then total error count.
                        entry_api = ctx.api or ""
                        trace_has_api: dict[str, bool] = {}
                        if entry_api:
                            for s in all_spans:
                                src = s.get("_source", s)
                                tid = src.get("traceId", "")
                                if tid and entry_api in str(src.get("name", "")):
                                    trace_has_api[tid] = True
                        best_tid = max(error_spans_by_trace, key=lambda t: (
                            1 if trace_has_api.get(t) else 0,
                            sum(1 for s in error_spans_by_trace[t]
                                if (s.get("_source", s).get("serviceName", "") or
                                    s.get("_source", s).get("resource.attributes.service@name", "")) in entity_services),
                            len(error_spans_by_trace[t]),
                        ))
                        fallback_qc["trace_id"] = best_tid
                        fallback_spans = adapter.get_traces(query_context=fallback_qc, data_dir=ctx.data_dir, case_id=ctx.case_id)
                        if fallback_spans:
                            spans = fallback_spans
                            ctx.query_context["trace_id"] = best_tid
                            execution_log.append(f"广域兜底：切换到含 {len(error_spans_by_trace[best_tid])} 个错误 span 的 traceId: {best_tid}")
        ...
        downstream_error_services = {
            ...
            and candidate.get("service") != entry_service
            and not candidate.get("is_propagation")
        }
        if downstream_error_services:
            for candidate in root_candidates:
                if candidate.get("service") != entry_service:
                    continue
                exception_type = str(candidate.get("exception_type") or "")
                root_type = str(candidate.get("type") or "")
                if exception_type not in {"HTTPError", "SlowRequest"} and root_type not in {"service_exception", "slow_interface"}:
                    continue
            ...
            for abnormal in abnormal_spans:
                if abnormal.get("service") != entry_service:
                    continue
                error_kind = str(abnormal.get("error_kind") or "")
                root_type = str(abnormal.get("root_cause_type") or "")
                if error_kind not in {"HTTPError", "SlowRequest"} and root_type not in {"service_exception", "slow_interface"}:
                    continue
        ...
        # ── Fix display: if the chosen candidate's API is a middleware client call
        # (Jedis/get, Druid/query, etc.), find the actual calling service from
        # call_path instead of using the first candidate's service.
        _MW_API_PATTERNS = ("jedis/", "redisson/", "lettuce/", "druid/", "hikari/", "jdbc/", "postgre/")
        _first_api_lower = (first_error_api or "").lower()
        if any(_first_api_lower.startswith(p) for p in _MW_API_PATTERNS):
            for p in call_path:
                if ":" in p:
                    svc, api_part = p.split(":", 1)
                    if api_part.strip() == first_error_api:
                        execution_log.append(f"[trace-fix] 中间件API {first_error_api} 匹配到 call_path 条目 {p!r}，服务从 {first_error_service} 改为 {svc.strip()}")
                        first_error_service = svc.strip()
                        break
            else:
                execution_log.append(f"[trace-fix] 中间件API {first_error_api} 未在 call_path 中找到匹配")
        ...
        return SkillResult(
            ...
            input={
                "data_file": (
                    resolution.data_source_label if resolution.skipped_adapter
                    else (
                        "opensearch:trace" if data_dir == "opensearch"
                        else "unifiedmodel:evidence/traces.json" if data_dir == "unifiedmodel"
                        else f"{data_dir}/trace.json"
                    )
                ),
                "api": ctx.api,
                "time": ctx.time,
