...
from app.runtime.log_input_resolver import resolve_log_input as _resolve_log_input
...
class LogAnalysisSkill(BaseSkill):
    ...
    def run(self, ctx: DiagnosisContext) -> SkillResult:
        ...
        # ── DCC-first: check whether DCC already supplies log records ─────────
        resolution = _resolve_log_input(ctx)
        for w in resolution.warnings:
            execution_log.append(w)

        if resolution.skipped_adapter:
            # Priority 1: use log records from DCC evidence.log.items
            data_dir = "dcc"
            logs = list(resolution.logs)
            execution_log.append(
                f"[DCC-first] 使用 DCC evidence.log.items，共 {len(logs)} 条日志"
            )
        else:
            # Priority 2 / 3 – Legacy / replay fallback
            # NOTE: the local_json branch is a *replay path* for demo evaluation
            # cases (examples/evaluation_cases/*). It is NOT the primary production
            # path. DCC or OpenSearch should be used in production.
            data_source = adapter.get_data_source()
            if data_source == "opensearch":
                data_dir = "opensearch"
            elif data_source == "local_json":
                # Legacy replay path: reads from examples/evaluation_cases/<case_id>/log.json
                data_dir = resolve_data_dir(data_dir=ctx.data_dir, case_id=ctx.case_id)
            else:
                data_dir = "unifiedmodel"

            execution_log.append(
                "查询 OpenSearch log" if data_dir == "opensearch"
                else ("读取 UnifiedModel logs" if data_dir == "unifiedmodel"
                      else f"[legacy-replay] 读取 {data_dir}/log.json")
            )
        ...
        downstream_url = None
        # ── Entity-first service filtering: primary filter from entity binding,
        # supplemented by trace call_path if trace analysis already ran.
        call_services = set()
        entity_services = ctx.entity_result.get("services", []) if isinstance(ctx.entity_result, dict) else []
        for s in entity_services:
            call_services.add(s)
            call_services.add(normalize_service_name(s))
        # Supplement: trace call_path services (if trace already executed)
        ...
        for log in logs:
            ...
            is_noise = (
                ...
                or (
                    # Apply stricter out-of-scope filter for online / DCC sources
                    # (not for legacy replay, where all logs in the file are scoped).
                    data_dir in ("opensearch", "dcc")
                    and call_services
                    and normalized_svc
            ...
            input={
                "data_file": (
                    resolution.data_source_label if resolution.skipped_adapter
                        ...
                        "opensearch:log" if data_dir == "opensearch"
                        else "unifiedmodel:evidence/logs.json" if data_dir == "unifiedmodel"
                        else f"[legacy-replay]{data_dir}/log.json"
                    )
                ),
                "candidate_services": ctx.entity_result.get("services", []),
            },
