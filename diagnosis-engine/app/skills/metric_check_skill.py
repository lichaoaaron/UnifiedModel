...
from app.runtime.metric_input_resolver import resolve_metric_input as _resolve_metric_input
...
class MetricCheckSkill(BaseSkill):
    ...
    def run(self, ctx: DiagnosisContext) -> SkillResult:
        ...
        # ── DCC-first: check whether DCC already supplies metric records ───────
        resolution = _resolve_metric_input(ctx)
        for w in resolution.warnings:
            execution_log.append(w)

        if resolution.skipped_adapter:
            # Priority 1: use metric records from DCC evidence.metric.items
            data_dir = "dcc"
            metrics = list(resolution.metrics)
            execution_log.append(
                f"[DCC-first] 使用 DCC evidence.metric.items，共 {len(metrics)} 条指标"
            )
        else:
            ...
            if data_source == "opensearch":
                ...
                # Legacy replay path: reads from examples/evaluation_cases/<case_id>/metric.json
                data_dir = resolve_data_dir(data_dir=ctx.data_dir, case_id=ctx.case_id)
            else:
                data_dir = "unifiedmodel"

            execution_log.append(
                "查询 OpenSearch metric" if data_dir == "opensearch"
                else ("读取 UnifiedModel metrics" if data_dir == "unifiedmodel"
                      else f"[legacy-replay] 读取 {data_dir}/metric.json")
            )
            ...
            input={
                "data_file": (
                    resolution.data_source_label if resolution.skipped_adapter
                        ...
                        "opensearch:metric" if data_dir == "opensearch"
                        else "unifiedmodel:evidence/metrics.json" if data_dir == "unifiedmodel"
                        else f"[legacy-replay]{data_dir}/metric.json"
                    )
                ),
